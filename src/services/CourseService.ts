...
async function fetchCourseFilter(course_code:string,teacher_name:string): Promise<Course[]> {
    const res = await fetch(`https://backend.ku-knowmore.xyz/courses/search/?code=${course_code}`);
    const courseInfo = await res.json();
    alert(courseInfo[0]);
    return courseInfo;  
}
async function fetchCourse(): Promise<Course_real[]> {
    const res = await fetch(`https://backend.ku-knowmore.xyz/courses`);
    const courseInfo = await res.json();
    return courseInfo;  
}

async function fetchCourseWithId(courseId:string): Promise<Course_real[]> {
    const res = await fetch(`https://backend.ku-knowmore.xyz/courses/${courseId}`);
    const courseInfo = await res.json();
    return courseInfo;
}
